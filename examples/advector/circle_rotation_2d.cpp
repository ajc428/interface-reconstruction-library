...
void CircleRotation2D::initialize(Data<double>* a_U, Data<double>* a_V,
                                  Data<double>* a_W,
                                  Data<IRL::PlanarSeparator>* a_separators) {
  CircleRotation2D::setVelocity(0.0, a_U, a_V, a_W);
  const BasicMesh& mesh = a_U->getMesh();
  const IRL::Pt circle_center(0.0, 0.25, mesh.zm(mesh.kmin()));
        ...
        if (IRL::magnitude(cell.calculateCentroid() - circle_center) -
          ...
          for (int ii = 0; ii < subdivisions; ++ii) {
            for (int jj = 0; jj < subdivisions; ++jj) {
              ...
              auto sub_cell_normal = IRL::Normal::fromPtNormalized(
              ...
              auto moments = IRL::getVolumeMoments<IRL::VolumeMomentsAndNormal>(
                  interface_poly);
              if (interface_poly.calculateNormal() != sub_cell_normal) {
                moments.normal() = -moments.normal();
              }
...
void CircleRotation2D::setVelocity(const double a_time, Data<double>* a_U,
