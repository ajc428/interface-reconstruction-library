...
void resetCentroids(const Data<IRL::LocalizedCylinderLink<double>>&
                        a_link_localized_cylinder,
                    Data<IRL::Pt>* a_liquid_centroid,
                    Data<IRL::Pt>* a_gas_centroid) {
  const BasicMesh& mesh = a_link_localized_cylinder.getMesh();
  for (int i = mesh.imino(); i <= mesh.imaxo(); ++i) {
    for (int j = mesh.jmino(); j <= mesh.jmaxo(); ++j) {
      for (int k = mesh.kmino(); k <= mesh.kmaxo(); ++k) {
        auto cell = IRL::RectangularCuboid::fromBoundingPts(
        ...
        auto moments = IRL::getNormalizedVolumeMoments<
            IRL::VolumeMoments>(
            cell, a_link_localized_cylinder(i, j, k).getNextReconstruction());
        (*a_liquid_centroid)(i, j, k) = moments.centroid();
        (*a_gas_centroid)(i, j, k)[0] = (mesh.xm(i) - moments.centroid()[0]*moments.volume()) / (1 - moments.volume());
        (*a_gas_centroid)(i, j, k)[1] = (mesh.ym(j) - moments.centroid()[1]*moments.volume()) / (1 - moments.volume());
        (*a_gas_centroid)(i, j, k)[2] = (mesh.zm(k) - moments.centroid()[2]*moments.volume()) / (1 - moments.volume());
      }
    }
  }
}
...
void connectMesh(
    const BasicMesh& a_mesh,
    Data<IRL::LocalizedCylinderLink<double>>* a_link_localized_cylinder) {
  IRL::LocalizedCylinderLink<double>* neighbor_ptr;
  // Provide mesh connectivity information.
  IRL::UnsignedIndex_t unique_id = 0;
  ...
  for (int i = a_mesh.imino(); i <= a_mesh.imaxo(); ++i) {
    for (int j = a_mesh.jmino(); j <= a_mesh.jmaxo(); ++j) {
      for (int k = a_mesh.kmino(); k <= a_mesh.kmaxo(); ++k) {
        (*a_link_localized_cylinder)(i, j, k).setId(unique_id);
        neighbor_ptr = i - 1 < a_mesh.imino()
                           ? nullptr
                           : &(*a_link_localized_cylinder)(i - 1, j, k);
        (*a_link_localized_cylinder)(i, j, k).setEdgeConnectivity(
            0, neighbor_ptr);
        neighbor_ptr = i + 1 > a_mesh.imaxo()
                           ? nullptr
                           : &(*a_link_localized_cylinder)(i + 1, j, k);
        ...
        neighbor_ptr = j - 1 < a_mesh.jmino()
                           ? nullptr
                           : &(*a_link_localized_cylinder)(i, j - 1, k);
        (*a_link_localized_cylinder)(i, j, k).setEdgeConnectivity(
            2, neighbor_ptr);
        neighbor_ptr = j + 1 > a_mesh.jmaxo()
                           ? nullptr
        ...
        neighbor_ptr = k - 1 < a_mesh.kmino()
                           ? nullptr
                           : &(*a_link_localized_cylinder)(i, j, k - 1);
        (*a_link_localized_cylinder)(i, j, k).setEdgeConnectivity(
            4, neighbor_ptr);
        neighbor_ptr = k + 1 > a_mesh.kmaxo()
                           ? nullptr
...
void advectVOF(
    ...
    Data<IRL::LocalizedCylinderLink<double>>* a_link_localized_cylinder,
    Data<double>* a_liquid_volume_fraction, Data<IRL::Pt>* a_liquid_centroid,
    Data<IRL::Pt>* a_gas_centroid, Data<IRL::Cylinder>* a_interface) {
  if (a_advection_method == "FullLagrangian") {
    FullLagrangian::advectVOF(a_reconstruction_method, a_dt, a_U, a_V, a_W,
                              a_link_localized_cylinder,
                              a_liquid_volume_fraction, a_liquid_centroid,
    ...
    SemiLagrangianCorrected::advectVOF(a_reconstruction_method, a_dt, a_U, a_V,
                                       a_W, a_link_localized_cylinder,
                                       a_liquid_volume_fraction,
                                       a_liquid_centroid, a_gas_centroid);
  } else if (a_advection_method == "Split") {
    Split::advectVOF(a_reconstruction_method, a_dt, a_U, a_V, a_W,
                     a_link_localized_cylinder, a_liquid_volume_fraction,
                     a_liquid_centroid, a_gas_centroid, a_interface);
  } else {
    std::cout << "Unknown advection method of : " << a_advection_method << '\n';
    std::cout << "Value entries are: FullLagrangian, SemiLagrangian, "
...
void Split::advectVOF(
  ...
  for (int dim = 0; dim < 3; ++dim) {
    getReconstruction(a_reconstruction_method, *a_liquid_volume_fraction,
                      *a_liquid_centroid, *a_gas_centroid,
                      *a_link_localized_cylinder, a_dt, a_U, a_V, a_W,
                      a_interface);
    ...
    for (int i = imin; i < imin + nx; ++i) {
      for (int j = jmin; j < jmin + ny; ++j) {
        for (int k = kmin; k < kmin + nz; ++k) {
          ...
          auto face_cell = IRL::RectangularCuboid::fromBoundingPts(x0, x1);
          IRL::Cylinder local_cylinder = (*a_interface)(i, j, k);
          (face_flux[dim])(i, j, k) =
              sign * IRL::getVolumeMoments<IRL::VolumeMoments>(
                         face_cell, (*a_interface)(i + shift[0], j + shift[1],
                                                   k + shift[2]));
  ...
  for (int i = imin; i < imin + nx; ++i) {
    for (int j = jmin; j < jmin + ny; ++j) {
      for (int k = kmin; k < kmin + nz; ++k) {
        ...
        // Now perform the actual cutting.
        auto cell_volume_moments =
            IRL::getNormalizedVolumeMoments<
                IRL::VolumeMoments>(
                transported_cell, (*a_link_localized_cylinder)(i, j, k));
        const double cell_volume = cell.calculateVolume();
        (*a_liquid_volume_fraction)(i, j, k) =
            cell_volume_moments.volume()/cell_volume;
        (*a_liquid_centroid)(i, j, k) = cell_volume_moments.centroid();
        (*a_gas_centroid)(i, j, k)[0] = (mesh.xm(i) - cell_volume_moments.centroid()[0]*cell_volume_moments.volume()) / (1 - cell_volume_moments.volume());
        (*a_gas_centroid)(i, j, k)[1] = (mesh.ym(j) - cell_volume_moments.centroid()[1]*cell_volume_moments.volume()) / (1 - cell_volume_moments.volume());
        (*a_gas_centroid)(i, j, k)[2] = (mesh.zm(k) - cell_volume_moments.centroid()[2]*cell_volume_moments.volume()) / (1 - cell_volume_moments.volume());
...
void SemiLagrangian::advectVOF(
    ...
    Data<IRL::Pt>* a_gas_centroid) {
  const BasicMesh& mesh = a_liquid_volume_fraction->getMesh();
  Data<IRL::VolumeMoments> face_flux[3] = {
      Data<IRL::VolumeMoments>(&mesh),
      Data<IRL::VolumeMoments>(&mesh),
      Data<IRL::VolumeMoments>(&mesh)};

  resetCentroids(*a_link_localized_cylinder, a_liquid_centroid,
                 a_gas_centroid);
        ...
        for (int dim = 0; dim < 3; ++dim) {
          ...
          (face_flux[dim])(i, j, k) =
              IRL::getVolumeMoments<IRL::VolumeMoments>(
                  face_cell[dim], (*a_link_localized_cylinder)(i, j, k));
        }
      }
    }
  }
  ...
  for (int i = imin; i < imin + nx; ++i) {
    for (int j = jmin; j < jmin + ny; ++j) {
      for (int k = kmin; k < kmin + nz; ++k) {
        int address = (i + mesh.getNgc()) * mesh.getNyo() * mesh.getNzo() +
                      (j + mesh.getNgc()) * mesh.getNzo() + (k + mesh.getNgc());
        face_flux_x_local[address] = (face_flux[0](i, j, k)).volume();
        face_flux_y_local[address] = (face_flux[1](i, j, k)).volume();
        face_flux_z_local[address] = (face_flux[2](i, j, k)).volume();
      }
    }
  }
  ...
  for (int i = mesh.imin(); i <= mesh.imax(); ++i) {
    for (int j = mesh.jmin(); j <= mesh.jmax(); ++j) {
      for (int k = mesh.kmin(); k <= mesh.kmax(); ++k) {
        int address = (i + mesh.getNgc()) * mesh.getNyo() * mesh.getNzo() +
                      (j + mesh.getNgc()) * mesh.getNzo() + (k + mesh.getNgc());
        (face_flux[0](i, j, k)).volume() = face_flux_x[address];
        (face_flux[1](i, j, k)).volume() = face_flux_y[address];
        (face_flux[2](i, j, k)).volume() = face_flux_z[address];
      }
    }
  }
  ...
  for (int i = imin; i < imin + nx; ++i) {
    for (int j = jmin; j < jmin + ny; ++j) {
      for (int k = kmin; k < kmin + nz; ++k) {
        int address = (i + mesh.getNgc()) * mesh.getNyo() * mesh.getNzo() +
                      (j + mesh.getNgc()) * mesh.getNzo() + (k + mesh.getNgc());
        face_flux_x_local[address] = 1-(face_flux[0](i, j, k)).volume();
        face_flux_y_local[address] = 1-(face_flux[1](i, j, k)).volume();
        face_flux_z_local[address] = 1-(face_flux[2](i, j, k)).volume();
      }
    }
  }
      ...
      for (int k = mesh.kmin(); k <= mesh.kmax(); ++k) {
        ...
        (*a_liquid_volume_fraction)(i, j, k) =
            (previous_liquid_volume_fraction * cell_volume +
             ...
             (face_flux[2])(i, j, k + 1).volume()) /
            (cell_volume + (face_flux[0])(i, j, k).volume() -
             (face_flux[0])(i + 1, j, k).volume() +
             (face_flux[1])(i, j, k).volume() -
             (face_flux[1])(i, j + 1, k).volume() +
             (face_flux[2])(i, j, k).volume() -
             (face_flux[2])(i, j, k + 1).volume() +
             (1-(face_flux[0])(i, j, k).volume()) -
             (1-(face_flux[0])(i + 1, j, k).volume()) +
             (1-(face_flux[1])(i, j, k).volume()) -
             (1-(face_flux[1])(i, j + 1, k).volume()) +
             (1-(face_flux[2])(i, j, k).volume()) -
             (1-(face_flux[2])(i, j, k + 1).volume()));
        ...
        if ((*a_liquid_volume_fraction)(i, j, k) <
          ...
          (*a_liquid_centroid)(i, j, k) =
              IRL::Pt(previous_liquid_volume_fraction * cell_volume *
                          (*a_liquid_centroid)(i, j, k) +
                      (face_flux[0])(i, j, k).centroid() -
                      (face_flux[0])(i + 1, j, k).centroid() +
                      (face_flux[1])(i, j, k).centroid() -
                      (face_flux[1])(i, j + 1, k).centroid() +
                      (face_flux[2])(i, j, k).centroid() -
                      (face_flux[2])(i, j, k + 1).centroid()) /
              (previous_liquid_volume_fraction * cell_volume +
               (face_flux[0])(i, j, k).volume() -
               (face_flux[0])(i + 1, j, k).volume() +
          ...
          // Update gas centroid, .centroid() is un-normalized
          IRL::Pt bary;
          bary[0] = (mesh.xm(i) - (face_flux[0])(i, j, k).centroid()[0]) / ((face_flux[0])(i, j, k).volume());
          bary[1] = (mesh.ym(j) - (face_flux[0])(i, j, k).centroid()[1]) / ((face_flux[0])(i, j, k).volume());
          bary[2] = (mesh.zm(k) - (face_flux[0])(i, j, k).centroid()[2]) / ((face_flux[0])(i, j, k).volume());

          IRL::Pt bary1;
          bary1[0] = (mesh.xm(i+1) - (face_flux[0])(i+1, j, k).centroid()[0]) / ((face_flux[0])(i+1, j, k).volume());
          bary1[1] = (mesh.ym(j) - (face_flux[0])(i+1, j, k).centroid()[1]) / ((face_flux[0])(i+1, j, k).volume());
          bary1[2] = (mesh.zm(k) - (face_flux[0])(i+1, j, k).centroid()[2]) / ((face_flux[0])(i+1, j, k).volume());

          IRL::Pt bary2;
          bary2[0] = (mesh.xm(i) - (face_flux[1])(i, j, k).centroid()[0]) / ((face_flux[1])(i, j, k).volume());
          bary2[1] = (mesh.ym(j) - (face_flux[1])(i, j, k).centroid()[1]) / ((face_flux[1])(i, j, k).volume());
          bary2[2] = (mesh.zm(k) - (face_flux[1])(i, j, k).centroid()[2]) / ((face_flux[1])(i, j, k).volume());

          IRL::Pt bary3;
          bary3[0] = (mesh.xm(i) - (face_flux[1])(i, j+1, k).centroid()[0]) / ((face_flux[1])(i, j+1, k).volume());
          bary3[1] = (mesh.ym(j+1) - (face_flux[1])(i, j+1, k).centroid()[1]) / ((face_flux[1])(i, j+1, k).volume());
          bary3[2] = (mesh.zm(k) - (face_flux[1])(i, j+1, k).centroid()[2]) / ((face_flux[1])(i, j+1, k).volume());

          IRL::Pt bary4;
          bary4[0] = (mesh.xm(i) - (face_flux[2])(i, j, k).centroid()[0]) / ((face_flux[2])(i, j, k).volume());
          bary4[1] = (mesh.ym(j) - (face_flux[2])(i, j, k).centroid()[1]) / ((face_flux[2])(i, j, k).volume());
          bary4[2] = (mesh.zm(k) - (face_flux[2])(i, j, k).centroid()[2]) / ((face_flux[2])(i, j, k).volume());

          IRL::Pt bary5;
          bary5[0] = (mesh.xm(i) - (face_flux[2])(i, j, k+1).centroid()[0]) / ((face_flux[2])(i, j, k+1).volume());
          bary5[1] = (mesh.ym(j) - (face_flux[2])(i, j, k+1).centroid()[1]) / ((face_flux[2])(i, j, k+1).volume());
          bary5[2] = (mesh.zm(k+1) - (face_flux[2])(i, j, k+1).centroid()[2]) / ((face_flux[2])(i, j, k+1).volume());
          (*a_gas_centroid)(i, j, k) =
              IRL::Pt((1.0 - previous_liquid_volume_fraction) * cell_volume *
                          (*a_gas_centroid)(i, j, k) +
                      bary -
