...
BasicMesh Film3D::setMesh(const IRL::UnsignedIndex_t a_n) {
...
void Film3D::initialize(Data<double>* a_U, Data<double>* a_V,
                               Data<double>* a_W,
                               Data<IRL::PlanarSeparator>* a_separators) {
  Film3D::setVelocity(0.0, a_U, a_V, a_W);
  const BasicMesh& mesh = a_U->getMesh();
  const IRL::Pt circle_center(0.5, 0.5, 0.5+mesh.dz()/2.0);//+mesh.dz()/2.0);
