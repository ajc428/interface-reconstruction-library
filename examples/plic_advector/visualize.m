...
for iteration = 0:number_of_viz_files-1
    ...
    liquid_volume_fraction(1:nx,1:ny,1:nz) = reshape(fscanf(datafileID,'%E',nx*ny*nz),[nz,ny,nx]);
    fclose(datafileID);
    isosurface(xm,ym,zm,liquid_volume_fraction);
    axis([x(1) x(nx+1) y(1) y(ny+1) z(1) z(nz+1)]);
    drawnow
    %colorbar;
    %caxis([0.0 1.0]);
