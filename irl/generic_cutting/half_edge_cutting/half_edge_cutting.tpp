...
#ifndef IRL_GENERIC_CUTTING_HALF_EDGE_CUTTING_HALF_EDGE_CUTTING_TPP_
#define IRL_GENERIC_CUTTING_HALF_EDGE_CUTTING_HALF_EDGE_CUTTING_TPP_
...
splitHalfEdgePolytope(SegmentedHalfEdgePolyhedronType *a_polytope,
                      ...
                      const Plane &a_plane, const double a_volume_tolerance) {
  splitHalfEdgePolytope(a_polytope, a_clipped_polytope, a_complete_polytope,
                        a_plane);
...
splitHalfEdgePolytope(SegmentedHalfEdgePolyhedronType *a_polytope,
  ...
  for (UnsignedIndex_t v = 0; v < starting_number_of_vertices; ++v) {
    ...
    if (vertex.isClipped()) {
      ...
      a_polytope->getVertexPointer(number_of_vertices_unclipped) = &vertex;
      ++number_of_vertices_unclipped;
    }
  }

  const auto vertices_after_intersection = a_polytope->getNumberOfVertices();
  const auto clipped_vertices_after_intersection =
      a_clipped_polytope->getNumberOfVertices();
  ...
  for (UnsignedIndex_t f = 0; f < clipped_faces; ++f) {
    auto starting_half_edge =
        *((*a_clipped_polytope)[f]->getStartingHalfEdge());
    ...
    auto new_opposite = a_complete_polytope->getNewHalfEdge(HalfEdgeType(
        made_half_edge->getPreviousVertex(), nullptr, nullptr, nullptr));
  ...
  for (UnsignedIndex_t v = clipped_vertices_after_intersection;
       v < a_clipped_polytope->getNumberOfVertices(); ++v) {
    auto current_vertex = a_clipped_polytope->getVertex(v);
    if (current_vertex->doesNotNeedToSeek()) {
      auto new_face_from_intersection = a_complete_polytope->getNewFace(
          FaceType(current_vertex->getHalfEdge()));
      ...
      do {
        ...
        auto next_half_edge = current_half_edge->getOppositeHalfEdge()
                                  ->getVertex()
                                  ->getHalfEdge();
  ...
  for (UnsignedIndex_t v = starting_number_of_vertices;
    ...
    if (current_vertex->doesNotNeedToSeek()) {
      ...
      do {
        ...
        auto current_half_edge = current_vertex->getHalfEdge();
        auto next_vertex =
            current_half_edge->getOppositeHalfEdge()->getVertex();
        current_half_edge->setFace(new_face_from_intersection);
        current_half_edge->getOppositeHalfEdge()->getFace()->markAsVisited();
  ...
  if (vertices_after_intersection > starting_number_of_vertices) {
    std::copy(&a_polytope->getVertexPointer(starting_number_of_vertices),
              &(a_polytope->getVertexPointer(vertices_after_intersection - 1)) +
                  1,
...
splitHalfEdgePolytope(SegmentedHalfEdgePolygonType *a_polytope,
  ...
  for (UnsignedIndex_t f = starting_number_of_faces;
       f < a_polytope->getNumberOfFaces(); ++f) {
    a_polytope->getFacePointer(number_of_faces_unclipped) = (*a_polytope)[f];
    ++number_of_faces_unclipped;
  }
  a_polytope->setNumberOfFaces(number_of_faces_unclipped);
...
truncateHalfEdgePolytope(SegmentedHalfEdgePolyhedronType *a_polytope,
  ...
  for (UnsignedIndex_t v = starting_number_of_vertices;
    ...
    // A new edge should now be formed between the first intersection we found
    // and the second intersection we found. We'll steal the
    // first_clipped_half_edge to do this. We will also generate the opposite
    // that will be needed for the new face by stealing the half edge after the
    // first intersection vertex
