...
PlanarSeparator R2PCommon<CellType, kColumns>::runOptimization(
  ...
  // LevenbergMarquardtScaled<R2PType, static_cast<int>(R2PType::columns_m)>
  //    lm_solver;
...
R2PCommon<CellType, kColumns>::getBestReconstruction(void) {
  return best_reconstruction_m;
}

template <class CellType, UnsignedIndex_t kColumns>
const PlanarSeparator &
R2PCommon<CellType, kColumns>::getGuessReconstruction(void) {
  return guess_reconstruction_m;
}

template <class CellType, UnsignedIndex_t kColumns>
const CellType &
R2PCommon<CellType, kColumns>::getCell(const UnsignedIndex_t a_cell) {
  return cells_to_cut_m[a_cell];
}

template <class CellType, UnsignedIndex_t kColumns>
const ReferenceFrame &
R2PCommon<CellType, kColumns>::getBestReferenceFrame(void) {
  return best_reference_frame_m;
}

template <class CellType, UnsignedIndex_t kColumns>
const ReferenceFrame &
R2PCommon<CellType, kColumns>::getGuessReferenceFrame(void) {
...
UnsignedIndex_t findCellPhase(const CellType &a_cell,
                              const PlanarSeparator &a_reconstruction) {
  // 0 - Liquid, 1 - Gas, 2 - Mixed
  const UnsignedIndex_t cell_phase =
      a_reconstruction[0].signedDistanceToPoint(a_cell[0]) <= 0.0 ? 0 : 1;
  if (cell_phase == 0) {
    for (const auto &plane : a_reconstruction) {
      for (const auto &vertex : a_cell) {
        if (plane.signedDistanceToPoint(vertex) > 0.0) {
          return 2;
        }
      }
    }
...
void R2PCommon<CellType, kColumns>::setWeightedGeometryVectorFromReconstruction(
  ...
  for (UnsignedIndex_t i = 0; i < cells_to_cut_m.size(); ++i) {
    ...
    // Hack for cheaply checking for single phase cells.
    // 0 - Liquid, 1 - Gas, 2 - Mixed
    UnsignedIndex_t phase = findCellPhase(cells_to_cut_m[i], a_reconstruction);
    auto moments_from_cut_cell =
        SeparatedMoments<VolumeMoments>::fromScalarConstant(0.0);
    if (phase == 0) {
      moments_from_cut_cell[0] =
          VolumeMoments::calculateMoments(&cells_to_cut_m[i]);
    } else if (phase == 1) {
      moments_from_cut_cell[1] =
          VolumeMoments::calculateMoments(&cells_to_cut_m[i]);
    } else {
      moments_from_cut_cell =
          getVolumeMoments<SeparatedMoments<VolumeMoments>,
                           ReconstructionDefaultCuttingMethod>(
              cells_to_cut_m[i], a_reconstruction);
    }
    moments_from_cut_cell.normalizeByVolume();
...
template <class R2PType> void R2PDebug<R2PType>::updateBestGuess(void) {
...
template <class CellType>
PlanarSeparator
R2P_2D1P<CellType>::solve(const R2PNeighborhood<CellType> &a_neighborhood,
                          const PlanarSeparator &a_reconstruction) {
  return this->runOptimization(this, a_neighborhood, a_reconstruction);
}
...
template <class CellType> void R2P_2D1P<CellType>::updateBestGuess(void) {
  this->saveBestGuess();
}
...
template <class CellType>
PlanarSeparator
R2P_3D1P<CellType>::solve(const R2PNeighborhood<CellType> &a_neighborhood,
                          const PlanarSeparator &a_reconstruction) {
  return this->runOptimization(this, a_neighborhood, a_reconstruction);
}
...
template <class CellType> void R2P_3D1P<CellType>::updateBestGuess(void) {
  this->saveBestGuess();
}
...
template <class CellType>
PlanarSeparator
R2P_2D2P<CellType>::solve(const R2PNeighborhood<CellType> &a_neighborhood,
                          const PlanarSeparator &a_reconstruction) {
  return this->runOptimization(this, a_neighborhood, a_reconstruction);
}
...
void R2P_2D2P<CellType>::updateGuess(
  ...
  UnitQuaternion rotation =
      this->getDeltaRotationQuat(this->best_reference_frame_m, *a_delta);
  this->guess_reference_frame_m = rotation * this->best_reference_frame_m;
  this->guess_distances_m[0] =
      this->best_distances_m[0] + (*a_delta)(2) * this->characteristic_length_m;
  this->guess_distances_m[1] =
      this->best_distances_m[1] + (*a_delta)(3) * this->characteristic_length_m;
...
template <class CellType> void R2P_2D2P<CellType>::updateBestGuess(void) {
  this->saveBestGuess();
...
template <class CellType> const double &R2P_2D2P<CellType>::getBestBeta(void) {
  return best_beta_m;
}
...
template <class CellType>
PlanarSeparator
R2P_3D2P<CellType>::solve(const R2PNeighborhood<CellType> &a_neighborhood,
                          const PlanarSeparator &a_reconstruction) {
  return this->runOptimization(this, a_neighborhood, a_reconstruction);
}
...
void R2P_3D2P<CellType>::updateGuess(
  ...
  this->guess_reference_frame_m = rotation * this->best_reference_frame_m;
  guess_distances_m[0] =
      best_distances_m[0] + (*a_delta)(4) * this->characteristic_length_m;
  guess_distances_m[1] =
      best_distances_m[1] + (*a_delta)(5) * this->characteristic_length_m;
...
template <class CellType> void R2P_3D2P<CellType>::updateBestGuess(void) {
  this->saveBestGuess();
  best_beta_m = guess_beta_m;
  best_distances_m[0] = guess_distances_m[0];
  best_distances_m[1] = guess_distances_m[1];
...
template <class CellType> const double &R2P_3D2P<CellType>::getBestBeta(void) {
  return best_beta_m;
}
