 ...
 public:
  ...
  Mesh(const int a_nx, const int a_ny, const int a_nz, const int a_ngc)
      : dx_m(0.0),
        ...
        jc_m(a_ny / 2),
        kc_m(a_nz / 2) {
    x_m.resize(static_cast<std::size_t>(a_nx + 1 + 2 * ngc_m));
    y_m.resize(static_cast<std::size_t>(a_ny + 1 + 2 * ngc_m));
    z_m.resize(static_cast<std::size_t>(a_nz + 1 + 2 * ngc_m));
  }
  ...
  Mesh(Mesh&& other) {
    ...
    jc_m = other.jc_m;
    kc_m = other.kc_m;
    x_m = std::move(other.x_m);
    y_m = std::move(other.y_m);
    z_m = std::move(other.z_m);
  }
  ...
  int ic(void) { return ic_m; }
  int jc(void) { return jc_m; }
  int kc(void) { return kc_m; }
