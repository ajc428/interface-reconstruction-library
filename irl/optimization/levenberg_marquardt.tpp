...
#ifndef IRL_OPTIMIZATION_LEVENBERG_MARQUARDT_TPP_
#define IRL_OPTIMIZATION_LEVENBERG_MARQUARDT_TPP_
...
void LevenbergMarquardt<OptimizingClass, kRows, kColumns>::solve(
  ...
  while (otype_m->errorTooHigh(error)) {
    ...
    for (UnsignedIndex_t i = 0; i < kColumns; ++i) {
      ...
      for (UnsignedIndex_t j = 0; j < kColumns; ++j) {
        A_m(i, j) *= precond;
      }
      rhs_precond_m(i) = rhs_m(i) * precond;
    }
    delta_m = A_m.completeOrthogonalDecomposition().solve(rhs_precond_m);
    ...
    double gain_ratio = (error - guess_error) /
                        (delta_m.transpose() * (damping_param * delta_m));
...
template <class OptimizingClass, int kRows, int kColumns>
LevenbergMarquardtNew<OptimizingClass, kRows, kColumns>::LevenbergMarquardtNew(
    void)
    : otype_m(nullptr) {}

template <class OptimizingClass, int kRows, int kColumns>
void LevenbergMarquardtNew<OptimizingClass, kRows, kColumns>::solve(
    OptimizingClass* a_setup_otype,
    const Eigen::Matrix<double, kColumns, 1>& a_jacobian_delta) {
  otype_m = a_setup_otype;
  this->solve(a_jacobian_delta);
...
template <class OptimizingClass, int kRows, int kColumns>
int LevenbergMarquardtNew<OptimizingClass, kRows, kColumns>::getReason(void) {
  return reason_for_exit_m;
}

template <class OptimizingClass, int kRows, int kColumns>
UnsignedIndex_t LevenbergMarquardtNew<OptimizingClass, kRows,
                                      kColumns>::getIterationCount(void) {
  return iteration_m;
}
...
void LevenbergMarquardtNew<OptimizingClass, kRows, kColumns>::solve(
  ...
  // has, which is why calculateVectorError() is called before this.
  vector_error_m = otype_m->calculateVectorError();
  // otype_m->calculateJacobian(&jacobian_transpose_m, &jacTjac_m);
  this->calculateJacobian(a_jacobian_delta, &jacobian_transpose_m, &jacTjac_m);
  ...
  while (true) {  // otype_m->errorTooHigh(error)) {
    iteration_m++;
    if (otype_m->iterationTooHigh(iteration_m)) {
      // Exiting because exceeding max iterations
      reason_for_exit_m = -1;
...
template <class OptimizingClass, int kRows, int kColumns>
void LevenbergMarquardtNew<OptimizingClass, kRows, kColumns>::calculateJacobian(
    const Eigen::Matrix<double, kColumns, 1>& a_delta,
    Eigen::Matrix<double, kColumns, kRows>* a_jacobian_transpose,
    Eigen::Matrix<double, kColumns, kColumns>* a_jacTjac) {
  // Set up temporary delta
  ...
  for (int parameter = 0; parameter < kColumns; ++parameter) {
    ...
    // solo_delta(parameter) = -a_delta(parameter);
    // otype_m->updateGuess(&solo_delta);
    // change_in_guess -= otype_m->calculateChangeInGuess();
    for (int elem = 0; elem < kRows; ++elem) {
      (*a_jacobian_transpose)(parameter, elem) =
          change_in_guess(elem) / safelyEpsilon(a_delta(parameter));
    }
  }
  // Calculate Tranpose(Jacobian) * Jacobian
  *a_jacTjac = (*a_jacobian_transpose) * (a_jacobian_transpose->transpose());
...
#endif  // IRL_OPTIMIZATION_LEVENBERG_MARQUARDT_TPP_